        ...
        if [ "${check_id}" -ne 0 ];then
    	    ...
    	    if [[ "${check_url}" -ne 0 ]];then
    		      item_id=`sed -n "${line_nu}p" ${m3u_file}|awk -F'tvg-name=' '{print$2}'|awk '{printf$1}' |sed 's/"//g'`
    			    ...
    			    echo "INSERT into tvbox.tv_channels(name,category,url) select '${item_id}','${item_group}','S
{item_url}' where NOT EXISTS (SELECT 1 FROM tvbox.tv_channels WHERE url = '${item_url}');" >> IPTV_update.sqltmp
    			    i=` expr $i + 1 `
    	    fi
    	fi
